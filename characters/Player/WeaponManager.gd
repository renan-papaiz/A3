...
enum WEAPON_SLOTS {HANDGUN, SHOTGUN, MACHINE_GUN}
var slots_unlocked = {
...
func init(_fire_point: Spatial, _bodies_to_exclude: Array):
	...
	weapons[WEAPON_SLOTS.HANDGUN].connect("fired", self, "alert_nearby_enemies")
...
func switch_to_weapon_slot(slot_ind: int):
	if slot_ind < 0 or slot_ind >= slots_unlocked.size() or slots_unlocked[slot_ind] == false:
		return
	if !slots_unlocked[cur_slot]:
		return
	disable_all_weapons()
